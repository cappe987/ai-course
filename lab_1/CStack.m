...
% 
% ¶¨ÒåÁËÒ»¸öÕ»
% s = CStack; ¶¨ÒåÒ»¸ö¿ÕµÄÕ»¶ÔÏó
% s = CStack(c); ¶¨ÒåÕ»¶ÔÏó£¬²¢ÓÃc³õÊ¼»¯s£¬µ±cÎªcellÊ±£¬cµÄÔªËØÎª¶ÓÁÐµÄÊý¾Ý£¬
%    ·ñÔòc±¾ÉíÎª¶ÓÁÐµÄµÚÒ»¸öÊý¾Ý
%
% Ö§³Ö²Ù×÷£º
%     sz = s.size() ·µ»ØÕ»ÄÚÔªËØ¸öÊý£¬Ò²¿ÉÓÃÀ´ÅÐ¶ÏÕ»ÊÇ·ñ·Ç¿Õ¡£
%     s.isempty() ÅÐ¶ÏÕ»ÊÇ·ñÎª¿Õ
%     s.empty() Çå¿ÕÕ»
%     s.push(el) ½«ÐÂÔªËØelÑ¹ÈëÕ»ÄÚ
%     s.pop()  µ¯³öÕ»¶¥ÔªËØ£¬ÓÃ»§Ðè×Ô¼ºÈ·±£Õ»·Ç¿Õ
%     el = s.top() ·µ»ØÕ»¶¥ÔªËØ£¬ÓÃ»§Ðè×Ô¼ºÈ·±£Õ»·Ç¿Õ
%     s.remove() Çå¿ÕÕ»
%     s.content() °´Ë³Ðò·µ»ØsµÄÊý¾Ý£¬ÎªÒ»¸öcellÊý×é
%
% See also CList, CQueue
%
